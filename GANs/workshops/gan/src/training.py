...
NUM_WORKERS = 4  # How many parallel workers for data ingestion. NOTE: Set to 0 when debugging
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        for i, real_img_batch in tqdm(enumerate(data_loader), total=total_iterations, desc=f"Epoch: {epoch}", unit="batches"):
            ...
            loss_c = \
                 critic(real_img_batch).mean() - critic(fake_img_batch).mean()
            ...
            if i % c_times == 0:
                ...
                ##################################################################
                """
                # gen_imgs = torch.rand((BATCH_SIZE, Z_SIZE))
